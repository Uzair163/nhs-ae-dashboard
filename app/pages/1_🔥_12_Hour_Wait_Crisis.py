...
is_filtered = scope != "England (all Trusts)"
...
if is_filtered:
    st.caption(
        "Note: the Winter 2026-27 forecast further down is calculated at national level only "
        "(the forecast model has no Trust breakdown), so it doesn't change with this filter."
    )
...
if jan26 is not None and dec22 is not None:
    st.error(
    ...
    st.info("Jan 2026 and/or Dec 2022 figures aren't available for the current filter selection.")
...
st.line_chart(full_chart, height=380, color=[theme.NHS_RED])
...
st.line_chart(pivot, height=380, color=[theme.NHS_BLUE, theme.NHS_LIGHT_BLUE])
